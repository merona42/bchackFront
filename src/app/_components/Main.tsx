...
export default function Main() {
    ...
    const handleSubmit = (currentQuestion: string) => {
        if (!currentQuestion.trim() || loading) return; // 빈 입력 또는 로딩 중인 경우 중복 요청 방지
        setLoading(true);
        ...
        fetch(`${apiUrl}/api/${content[selectedItem].endpoint}`, {
            method: 'POST',
            headers: {
                ...
                <div className={style.companyMenu}>
                    <ul style={{ listStyleType: "none", paddingLeft: 0 }}>
                        {Object.keys(content).map((item) => (
                            ...
                            >
                                <img src={content[item].img} alt="company" />
                ...
                <div className={style.rightBody}>
                    <div className={style.bodyHeader}>
                        <div className={style.bodyHeaderContainer}>
                            <img src={content[selectedItem].img} alt="company_logo" />
                            <div className={style.bodyHeaderContents}>
                                <h3>{content[selectedItem].title}</h3>
                                <div>{content[selectedItem].description}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={style.chatArea}>
                        {messages.map((m) => (
                            <div key={m.messageId} className={m.id === 'ai' ? cx(style.message, style.aiMessage) : cx(style.message, style.myMessage)}>
                    ...
                    <div className={style.questionAreaContainer}>
                        <div className={style.questionArea}>
                            ...
                            />
                            <span onClick={() => handleSubmit(question)}>
